 ...
 * 画面仕様:
 * - PC限定表示
 * - GET /properties/{id} でデータをロード
 * - フォームに初期値を設定
 * - 更新ボタンで PUT /properties/{id} を呼び出し
 *
 * 認可: internal / admin
 */
...
export default function PropertyEdit() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      // URLパスから物件IDを抽出: /properties/{id}/edit
      const pathMatch = window.location.pathname.match(/\/properties\/([^\/]+)\/edit/);
      ...
      try {
        ...
        if (response.status === 'success' && response.data) {
          ...
          setFormData({
            ...
            typeLarge: property.typeLarge || '',
            typeMedium: property.typeMedium || '',
            typeSmall: property.typeSmall || '',
        ...
        setSubmitError(err instanceof Error ? err.message : 'データの読み込みに失敗しました');
      } finally {
        setLoading(false);
      }
    };
  ...
  const submitForm = async (redirectOverride?: string) => {
    ...
    try {
      ...
      submitData.zipcode = formData.zipcode.replace(/-/g, '');
      submitData.prefecture = formData.prefecture;
      submitData.city = formData.city.trim();
      submitData.address = formData.address.trim();
      submitData.typeLarge = formData.typeLarge;

      // RBS・物件区分は「空欄に戻す」操作も反映させたいので、空でも必ず送る
      submitData.rbs = formData.rbs;
      submitData.ownershipType = formData.ownershipType;
  ...
  // ========================================
  const handleCancel = () => {
    handleNavigate(`/properties/${propertyId}`);
  };
  ...
  const handleDelete = async () => {
    if (!confirm(`物件「${originalData?.name}」を削除してもよろしいですか？\nこの操作は取り消せません。`)) {
      return;
    }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-2 py-2.5 md:flex-row md:items-center md:justify-between md:gap-3 md:py-0 md:h-16">
            <div className="flex items-center gap-3 overflow-x-auto -mx-1 px-1 sm:gap-4 md:overflow-visible md:mx-0 md:px-0">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-800 whitespace-nowrap">物件編集</h1>
                <p className="text-xs text-slate-500">ID: {propertyId}</p>
              </div>
            </div>
            {/* 削除ボタン */}
            <button
              onClick={handleDelete}
              className="shrink-0 whitespace-nowrap inline-flex items-center px-4 py-2 text-sm font-medium text-red-600 bg-red-50 border border-red-200 rounded-lg hover:bg-red-100"
            >
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  value={propertyId}
                  disabled
                  className="w-full px-4 py-2.5 border border-slate-300 rounded-lg bg-slate-100 text-slate-500"
                />
                <p className="mt-1 text-xs text-slate-500">物件IDは変更できません</p>
              </div>
          ...
          <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <p className="text-sm text-slate-600 mb-4">
              写真、図面、PDF、動画などのファイルをアップロードできます。
            </p>
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => handleNavigate(`/properties/${propertyId}/?tab=files`)}
                className="inline-flex items-center px-4 py-2.5 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => handleNavigate(`/properties/${propertyId}/?tab=files`)}
                className="inline-flex items-center px-4 py-2.5 text-sm font-medium text-green-700 bg-green-50 border border-green-200 rounded-lg hover:bg-green-100 transition-colors"
              >
              ...
              <button
                type="button"
                onClick={() => handleNavigate(`/properties/${propertyId}/?tab=files`)}
                className="inline-flex items-center px-4 py-2.5 text-sm font-medium text-purple-700 bg-purple-50 border border-purple-200 rounded-lg hover:bg-purple-100 transition-colors"
              >
                <MovieIcon className="w-4 h-4 mr-2" />
                動画を管理
              </button>
            </div>
          </section>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm text-slate-600">
                <div>
                  ...
                  {originalData.createdAt
                    ? new Date(originalData.createdAt).toLocaleString('ja-JP')
                    : '-'}
                </div>
                <div>
                  <span className="text-slate-500">更新日時:</span>{' '}
                  {originalData.updatedAt
                    ? new Date(originalData.updatedAt).toLocaleString('ja-JP')
                    : '-'}
                </div>
              </div>
            </section>
